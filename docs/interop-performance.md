...
`tools/performance.py` runs five independent uninstrumented engine sessions over 64 KiB, 1 MiB and 16 MiB bodies. Each execution verifies exact acknowledgement counts and a hard 32 KiB engine output queue bound while writes accept only 997 bytes at a time. Reports include allocation totals and post-major live words. These are payload/heap observations, not a claim that total process RSS equals queue capacity.
