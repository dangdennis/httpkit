...
- [Native generated-input campaigns](native-fuzz.md) run the full shared target
  ...
  smoke evidence and longer campaigns are recorded separately. Property failures
  capture exact generated bytes without overwriting earlier evidence; bounded
  raw-input replay snapshots the chosen input and records its hash. Capture
  failures preserve the original exception, and timeouts may have no raw capture.
  Native byte-input minimization now preserves the exception class/backtrace,
  reproduces accepted reductions twice, retains original/best inputs and logs,
  and distinguishes budget exhaustion from completed deletion minimality.
  Release-duration acceptance remains open; AFL stays skipped.
