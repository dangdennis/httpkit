...
Queue counters report engine-owned body payload. Codec metadata, queue nodes, bookkeeping and adapter staging add bounded overhead. Application-retained events are outside engine ownership. Independent instances may run on different domains; one instance must not be mutated concurrently.
