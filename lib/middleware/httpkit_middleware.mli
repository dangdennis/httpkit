...
module Basic : sig
  type ('body, 'output) handler = 'body Httpkit_core.Request.t -> 'output
...
module Context : sig
  type ('context, 'body, 'output) handler =
    'context -> 'body Httpkit_core.Request.t -> 'output
...
module Transition : sig
  ...
  val guard :
    ('before -> 'body Httpkit_core.Request.t -> ('after, 'error) result) ->
    reject:('error -> 'output) ->
