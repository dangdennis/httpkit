...
with tempfile.TemporaryDirectory() as directory:
    ...
    assert check()['status']=='READY','positive control rejected'
    for name,fields in [
      ...
      ('compiler-5.5.0',dict(compiler='5.2.1'))]:
        ...
        assert check()['status']=='NOT_READY',name;path.write_bytes(original)
    path=root/'coverage.json';data=json.loads(path.read_text());data['source_sha256']='old';path.write_text(json.dumps(data))
    assert check()['status']=='NOT_READY'
    path.write_text('{broken');assert check()['status']=='NOT_READY'
