...
mutants=[
 ('framing-cl-te','lib/http1/http_kit_http1.ml','if cl <> [] && te <> [] then Error Ambiguous_framing','if false && cl <> [] && te <> [] then Error Ambiguous_framing','test/http1/http1_test.exe'),
 ('foreign-request-id','lib/engine/http_kit_engine.ml','a.owner == b.owner && a.number = b.number','a.owner == a.owner && a.number = b.number','test/engine/engine_test.exe'),
 ('output-accounting','lib/engine/http_kit_engine.ml','t.queued <- t.queued - count;','t.queued <- t.queued - min count 0;','test/engine/engine_test.exe'),
]
results=[]
with tempfile.TemporaryDirectory(prefix='http-kit-mutants-') as directory:
    stage=Path(directory)
    for name in ['lib','test','fuzz','examples','bench']:shutil.copytree(ROOT/name,stage/name)
    for path in ROOT.glob('*.opam'):shutil.copy2(path,stage/path.name)
    shutil.copy2(ROOT/'dune-project',stage/'dune-project')
    (stage/'dune-workspace').write_text('(lang dune 3.24)\n(pkg disabled)\n')
    def build(target):
        p=subprocess.run([dune,'build',target],cwd=stage,env=clean,capture_output=True,timeout=180)
        if p.returncode:raise RuntimeError('mutant compilation is not a test kill:\n'+p.stderr.decode())
    def execute(target):
