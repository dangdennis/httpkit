...
with tempfile.TemporaryDirectory(prefix='httpkit-router-') as directory:
    ...
    for args in [['build', '@install'], ['install', '--prefix', str(prefix), 'httpkit-core', 'httpkit-router']]:
        subprocess.run([dune, *args], cwd=source, env=clean, check=True, capture_output=True, timeout=120)
    require(sorted(p.name for p in (prefix / 'lib').iterdir()) == ['httpkit-core', 'httpkit-router'], "test_router_consumer.py: sorted(p.name for p in (prefix / 'lib').iterdir()) == ['httpkit-core', 'httpkit-router']")
    includes = [str(prefix / 'lib' / name) for name in ['httpkit-core', 'httpkit-router']]
    ...
    for mode, tool, extension in [('byte', compiler, 'cma'), ('native', compiler.with_name('ocamlopt'), 'cmxa')]:
        args = [str(tool), *[x for path in includes for x in ['-I', path]],
                ...
                str(Path(includes[1]) / ('httpkit_router.' + extension)), 'consumer.ml', '-o', 'consumer-' + mode]
    ...
    require(result.returncode != 0 and 'Httpkit_router.pattern' in result.stderr, "test_router_consumer.py: result.returncode != 0 and 'Httpkit_router.pattern' in result.stderr")
