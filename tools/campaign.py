...
def main():
    ...
    revision = (ROOT / "toolchain/afl.version").read_text().split()[2]
    require(
        subprocess.check_output(
            ["git", "-C", str(afl), "rev-parse", "HEAD"], text=True
        ).strip()
        == revision,
        "campaign.py: subprocess.check_output(['git','-C',str(afl),'rev-parse','HEAD'],text=True).strip()==revision",
    )
    ...
    for flags in [[], ["--profile", "fuzz", "--build-dir", build]]:
        subprocess.run(
            command(dune, env, ["build", *flags, "-j", "4", *binaries]),
            cwd=ROOT,
            env=env,
            check=True,
            timeout=1800,
        )
    base = ROOT / "_artifacts/campaigns"
    ...
    for target in targets:
        name = target["name"]
        target_env = dict(env)
        if target["case"] is not None:
            target_env["HTTP_KIT_FUZZ_CASE"] = target["case"]
        seeds = directory / (name + "-seeds")
        seeds.mkdir()
        ...
        require(
            result.returncode == 0,
            ("AFL failed; logs and corpus retained", name, directory),
        )
        findings = [
            p for p in output.rglob("id:*") if p.parent.name in ("crashes", "hangs")
        ]
        require(
            not findings,
            ("untriaged findings retained", name, [str(p) for p in findings]),
        )
        stats_files = list(output.rglob("fuzzer_stats"))
        require(len(stats_files) == 1, "campaign.py: len(stats_files)==1")
        stats = {
            k.strip(): v.strip()
            for k, v in (
                line.split(":", 1)
        ...
        seconds = int(stats["run_time"])
        executions = int(stats["execs_done"])
        require(
            seconds >= max(1, args.seconds - 2) and executions >= 10,
            ("incomplete campaign", name, stats),
        )
        ...
        row = {
            ...
            "uninstrumented_replays": len(corpus),
        ...
        require(
            source_hash() == digest,
            "sources changed during target; refusing to record evidence",
        )
        record(
            "campaign-" + name + ".json",
            {"status": "PASS", "compiler": version, "afl_revision": revision, **row},
        )
        print(json.dumps(row), flush=True)
    require(
        source_hash() == digest, "sources changed during campaign; evidence invalid"
    )
    record(
