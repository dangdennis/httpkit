...
class OperationalChecks(unittest.TestCase):
    def test_production_guards(self):
        ...
        fixtures = {
            'mutations.py': [
                ('baseline.returncode', dict(baseline=SimpleNamespace(returncode=1, stdout=b'', stderr=b''))),
                ('original.count', dict(original='', before='missing', name='drift')),
                ('result.returncode', dict(result=SimpleNamespace(returncode=0, stdout=b'', stderr=b''), name='survivor')),
                ('source_hash()', dict(source_hash=lambda: 'new', digest='old')),
            ],
            'campaign.py': [
                ('result.returncode', dict(result=SimpleNamespace(returncode=1), name='test', directory='logs')),
                ('not findings', dict(findings=['finding'], name='test')),
                ('seconds>=', dict(seconds=1, executions=100, args=SimpleNamespace(seconds=30), name='test', stats={})),
                ('seconds>=', dict(seconds=30, executions=0, args=SimpleNamespace(seconds=30), name='test', stats={})),
            ],
        }
        for filename, cases in fixtures.items():
            ...
            for fragment, values in cases:
                call = next(node for node in calls if fragment in ast.get_source_segment(source, node.args[0]))
                for optimization in (0, 1, 2):
                    with self.subTest(file=filename, guard=fragment, optimization=optimization):
                        code = compile(ast.Expression(call), filename, 'eval', optimize=optimization)
    ...
    def test_subprocess_optimization(self):
        ...
        for flags, extra in (([], {}), (['-O'], {}), ([], {'PYTHONOPTIMIZE': '1'})):
            result = subprocess.run([sys.executable, *flags, '-c', code],
                                    cwd=Path(__file__).parent, env={**os.environ, **extra},
