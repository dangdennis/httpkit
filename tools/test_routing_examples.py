...
for runtime in ["eio", "lwt"]:
    with tempfile.TemporaryFile() as errors:
        binary = (
            ROOT
            / ("_build-pkg-" + env["HARNESS_COMPILER"])
            / "default/examples/routing"
            / (runtime + "_server.exe")
        )
        server = subprocess.Popen(
            [str(binary)], cwd=ROOT, env=env, stdout=subprocess.PIPE, stderr=errors
        )
        connection = None
        try:
            ...
            require(
                url.scheme == "http" and url.hostname == "127.0.0.1" and url.port,
                "test_routing_examples.py: url.scheme == 'http' and url.hostname == '127.0.0.1' and url.port",
            )
            ...
            for method, target, body, status, expected, allow in cases:
                ...
                require(
                    (response.status, actual) == (status, expected),
                ...
                require(
                    ...
                    "test_routing_examples.py: response.getheader('allow') == allow",
                )
            for headers, status, expected in [
                ({}, 401, b"Demo identity required\n"),
                ({"x-demo-user": "demo"}, 200, b"Hello demo\n"),
            ]:
                connection.request("GET", "/protected", headers=headers)
                response = connection.getresponse()
                require(
                    (response.status, response.read()) == (status, expected), runtime
                )
            for target, status in [("/echo", b"100"), ("/missing", b"404")]:
                with socket.create_connection(
                    ...
                    while True:
                        ...
                        require(line, "EOF before head ended")
                        if line == b"\r\n":
                            break
                    if status == b"100":
                        peer.sendall(b"body")
                        ...
                        while True:
                            line = stream.readline()
                            require(line, "EOF before final head ended")
                            if line == b"\r\n":
                                break
            ...
            print(
                "PASS:",
                runtime,
                "routing/middleware over persistent HTTP, including HEAD, 404/405 and raw captures",
            )
        finally:
