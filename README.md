...
The [code quality consolidation plan](docs/code-quality-plan.md) records the review fixes and improvements to readability, API ergonomics, protocol contracts and benchmark evidence. Native adapters expose engine limits and formatted failures; the routing examples include asynchronous Transition composition.
...
`mise run bench:bodies` compares public request/response body readers across framing, payload size, fragmentation and consumption variants. `mise run bench:router-experiment` checks and measures a candidate index against the reference matcher. `mise run bench:exchanges` adds response writers and persistent pipelines. `mise run bench:profile-bodies` captures separate allocation/driver/memory diagnostics. Both router index prototypes remain benchmark-only. Use `--min-ms 50` on the benchmark runner for calibrated batches; short/noisy cases remain unsuitable for ranking. These lanes are also included in `bench:compare`; framing mismatches are retained as explicit exclusions, without timing ratios. See the persistent [benchmark TODO plan](docs/benchmark-todos.md) for remaining coverage and adoption gates, and the [follow-up findings](docs/benchmark-results-2026-09-11.md) for measured tradeoffs.
